...
We measured the computational time of the FFT analysis and the WT analysis for different ROI sizes on 77 videos of 5 seconds, and
averaged the results. The ROI sizes tested were $1$, $[3, 4]$ and $[6, 8]$. The results are plotted in Figure 

\pgfplotstableread{speed_results_partial.dat}\speed
\pgfplotstabletranspose[Rows from=Rows]\speedT{\speed}
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot table[y = FFT] from \speedT ; 
      \addlegendentry{FFT} ;
      \addplot table[y = WT] from \speedT ;
      \addlegendentry{WT} ;    
    \end{axis}
  \end{tikzpicture}
